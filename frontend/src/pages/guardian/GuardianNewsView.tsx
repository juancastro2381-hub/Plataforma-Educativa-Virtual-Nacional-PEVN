/**
 * PEVN Frontend — Guardian News View (Phase 15)
 *
 * Institutional news, school bulletin board, achievements, and events
 * for the parent & guardian community.
 */
...
export const GuardianNewsView: React.FC<Props> = ({ onBackToDashboard: _onBackToDashboard }) => {
  ...
  const fetchNews = async () => {
    ...
    setError(null)
    try {
      const res = await communicationApi.getGuardianNews()
      setNewsList(res.items)
    } catch (err: unknown) {
      const msg = err instanceof Error ? err.message : 'Error al cargar las noticias escolares.'
      setError(msg)
    } finally {
      setLoading(false)
    }
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
        ...
        <div>
          <div style={{ display: 'flex', alignItems: 'center', gap: '0.6rem' }}>
            ...
            <h2 style={{ margin: 0, fontSize: '1.25rem', fontWeight: 800, color: '#0F172A' }}>
              Periódico Escolar y Noticias
            </h2>
          </div>
          <p style={{ margin: '0.35rem 0 0 0', color: '#64748B', fontSize: '0.875rem' }}>
            Logros estudiantiles, eventos culturales, deportivos y vida institucional.
          </p>
        </div>
      ...
      {/* Featured Spotlight Banner if on "ALL" */}
      ...
      {loading ? (
        <div style={{ textAlign: 'center', padding: '3rem', color: '#64748B' }}>
          ...
          <p>Cargando noticias institucionales...</p>
        </div>
        ...
        >
          ...
          <p style={{ margin: 0, color: '#64748B', fontSize: '0.875rem' }}>
            Las noticias y reportajes escolares aparecerán aquí a medida que el equipo docente y directivo las publique.
          </p>
        </div>
      ) : (
        /* News Cards Grid */
