/**
 * PEVN Frontend — Guardian Communications View (Phase 15)
 *
 * Official institutional circulars, rectoral resolutions, and notifications
...
export const GuardianCommunicationsView: React.FC<Props> = ({ onBackToDashboard: _onBackToDashboard }) => {
  ...
  const fetchCommunications = async () => {
    ...
    setError(null)
    try {
      const res = await communicationApi.getGuardianCommunications()
      setCommunications(res.items)
      setUnreadCount(res.unread_count)
    } catch (err: unknown) {
      const msg = err instanceof Error ? err.message : 'Error al cargar los comunicados.'
      setError(msg)
    } finally {
      setLoading(false)
    }
  ...
  const handleAcknowledge = async (item: InstitutionalCommunicationItem, e: React.MouseEvent) => {
    ...
    try {
      const updated = await communicationApi.acknowledgeGuardianCommunication(item.id)
      ...
      const msg = err instanceof Error ? err.message : 'Error al registrar la confirmación de lectura.'
      alert(msg)
    } finally {
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
      ...
      >
        <div>
          ...
          <p style={{ margin: '0.35rem 0 0 0', color: '#64748B', fontSize: '0.875rem' }}>
            Circulares oficiales, convocatorias y notificaciones de rectoría dirigidas a acudientes.
          </p>
        </div>
      ...
      {loading ? (
        <div style={{ textAlign: 'center', padding: '3rem', color: '#64748B' }}>
          ...
          <p>Cargando comunicados institucionales...</p>
        </div>
      ) : filtered.length === 0 ? (
        /* Empty state */
        <div
        ...
        >
          ...
          <p style={{ margin: 0, color: '#64748B', fontSize: '0.875rem' }}>
            Los comunicados que hayan alcanzado su fecha límite de expiración se archivan de manera automática.
          </p>
        </div>
      ) : (
              ...
              >
                ...
                <div>
                  ...
                  >
                    ...
                    <span>
                      {item.published_at ? new Date(item.published_at).toLocaleDateString('es-CO') : ''}
                    </span>
                  </div>
      ...
      {selectedItem && (
        ...
        >
          ...
          >
            ...
            {/* Acknowledgment Action in Modal */}
